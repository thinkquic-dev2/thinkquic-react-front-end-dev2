...
const ConversationList = (props) => {
  ...
  const getReceiverName = (convname) => {
    ...
    if (user1.toLowerCase() !== currentUser.username.toLowerCase())
      return user1;
    if (user2.toLowerCase() !== currentUser.username.toLowerCase())
      return user2;
  };
        ...
        {currUserId ? (
          <Connect
            query={graphqlOperation(
              queries.getUserConversationConnectionThroughUser
            )}
          ...
          >
            {({ data, loading, error }) => {
              ...
              return userConversations.map((userC, i) => (
                <div
                  className={`convItem ${
                    conId === userC.conversationId ? "active" : ""
                  }`}
                  onClick={(e) => {
