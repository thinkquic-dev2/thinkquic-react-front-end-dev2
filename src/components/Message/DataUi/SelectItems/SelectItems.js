...
const SelectItems = ({
  ...
  const dropdownItems = items
    ...
    .map((dropdownItem, index) => (
      <option
        className="select-items__dropdown-item"
        value={dropdownItem}
  ...
  const selected = selectedItems.map((selectedItem, index) => (
    <span
      className="select-items__selected-item"
      key={"selected-item-" + index}
    >
      <span className="select-items__selected-item-text">{selectedItem}</span>
      <button
        className="select-items__remove-selected-item"
        onClick={() => removeItemClickhandler(selectedItem)}
      >
        x
      </button>
    </span>
