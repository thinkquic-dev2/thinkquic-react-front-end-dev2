            ...
            this.advanceConversation = function () {
              audioControl.supportsAudio(function (supported) {
                if (supported) {
                  ...
                  console.log("here in advance conversation");
          ...
          var Sending = function (state) {
            ...
            this.advanceConversation = function () {
              ...
              lexruntime.postContent(
                { ...state.lexConfig, sessionAttributes: window.sessionAttributes },
                function (err, data) {
                  if (err) {
                    state.onError(err);
          ...
          var Speaking = function (state) {
            ...
            this.advanceConversation = function () {
              ...
              if (state.audioOutput.contentType === "audio/mpeg") {
                if (isMessageJson(state.audioOutput.message)) {
                  const parsedMessage = JSON.parse(state.audioOutput.message);
                  const textMessages = parsedMessage.Result.Text;
                  const messageToRead = [];
                  for (var i = 0; i < textMessages.length; i++) {
                    const key = Object.keys(textMessages[i])[0];
                    messageToRead.push(textMessages[i][key].value);
                  }
                  const synthesizeSpeech = (text) => {
                    ...
                    polly.synthesizeSpeech(params, function (err, data) {
                      if (err) {
                        ...
                        audioControl.play(data.AudioStream, function () {
                          if (
                  ...
                  synthesizeSpeech("<speak>" + messageToRead.join(". ") + "</speak>");
                } else {
